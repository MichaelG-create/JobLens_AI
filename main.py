...
with open(csv_file, mode="a", newline="", encoding="utf-8") as file:
    ...
    while page_num <= max_page:
        page_url = (
            f"https://www.jobs.ch/fr/offres-emplois/?page={page_num}&term=data+engineer"
        )
        ...
        for offer in offers:
            try:
                offer_link = offer.find_element(
                    By.CSS_SELECTOR, "a[data-cy='job-link']"
                )
                ...
                date_published = format_date(
                    offer.find_elements(By.CLASS_NAME, "mb_s12")[0].text
                )  # Date publiée
                location = offer.find_elements(By.CLASS_NAME, "mb_s12")[
                    1
                ].text  # Localisation
                full_time = offer.find_elements(By.CLASS_NAME, "mb_s12")[
                    2
                ].text  # Temps plein
                contract = offer.find_elements(By.CLASS_NAME, "mb_s12")[
                    3
                ].text  # Contrat
                company = offer.find_elements(By.CLASS_NAME, "mb_s12")[
                    4
                ].text  # Entreprise
                ...
                if unique_key not in existing_keys:
                    ...
                    existing_keys.add(
                        unique_key
                    )  # Ajouter la clé unique à l'ensemble pour éviter les doublons
                else:
                    print(
                        f"Doublon trouvé, l'offre '{title}' de '{company}' est déjà présente."
                    )
        ...
        if page_num == 1:
            try:
                page_indicator_element = driver.find_element(
                    By.CSS_SELECTOR, "span[title^='Page']"
                )  # Sélecteur qui match avec les spans "Page X"
                page_text = (
                    page_indicator_element.text
                )  # Extraire le texte de l'élément, comme " ... 84"

                # Utiliser une expression régulière pour extraire le numéro de page à partir du texte
                match = re.search(r"\.\.\. (\d+)", page_text)  # Chercher "... XXX"
                if match:
                    last_page_number = match.group(1)  # Le numéro de la dernière page
                    print(f"Le numéro de la dernière page est : {last_page_number}")
                    max_page = int(last_page_number)
                else:
                    print("Le numéro de la dernière page n'a pas pu être extrait.")
